...
import ImageContainerNotation, {
  ImageContainerNotationRef,
} from '../../components/image-container-notation/image-container-notation';
import {
  BoundingBoxes,
  useMutationAnnotationQuery,
} from '../../api/useMutationAnnotationQuery';
import SearchBar from '../../components/searchbar/searchbar';
import OptionList, {
  OptionsListRef,
} from '../../components/option-list/option-list';
...
function App() {
  ...
  const {
    category: {onSetCategory, searchList, onSearch},
  } = createCategorySlice();
  const {
    images: {list, onSetImages, onMoveToAnalyzedImageList},
  } = createImagesSlice();
  ...
  useEffect(() => {
    if (imagesData) onSetImages(imagesData);
  }, [imagesData, onSetImages]);

  useEffect(() => {
    if (categoryData) onSetCategory(categoryData);
  }, [categoryData, onSetCategory]);
  ...
  return (
    <div className="main-content">
      ...
      <div className="analyzer-container">
        <ImageContainerNotation
          ref={imageContainerNotation}
          onSelectSquare={onSelectSquare}
          url={list.length ? currentImage.url : ''}
        />
