...
const ImageContainerNotation = forwardRef<
  ImageContainerNotationRef,
  IImageContainerNotation
>(({url, onSelectSquare}, ref) => {
  const [position, setPosition] = useState<BoundingBoxes>({
    width: 0,
    height: 0,
    topLeftX: 0,
    topLeftY: 0,
  ...
  const handleMouseUp = (e: MouseEvent) => {
    ...
    if (
      typeof bouncingPosition?.x === 'number' &&
      typeof bouncingPosition.y === 'number'
    ) {
